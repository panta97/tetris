...
class Animation {
  private stateDelete = {
    blinkingColors: [Group.orange, Group.yellow],
    pixels: [] as Pixel[][],
    step: 0,
    untilStep: 3
  }
  ...
  deleteFullRows(rowsToDelete: number[], board: Board) {
    ...
    // there are rows and step is starting step = 0
      ...
      state.pixels.forEach((row: Pixel[]) => {
        row.forEach((px: Pixel) => px.group = state.blinkingColors[state.step % 2])
      });
      ...
      // set color
      state.pixels.forEach((row: Pixel[]) => {
        row.forEach((px: Pixel) => px.group = state.blinkingColors[state.step % state.blinkingColors.length])
      });
